	...
	public class EventsController : ControllerBase
	{
		...
		public async Task<ActionResult<EventDTO>> AttendEvent(int eventId)
		{
			try
			{
				...
				// Return a message with a button to go back to the user dashboard
				var eventAttendance = new Event_Attendance 
				{
					EventId = eventId,
					Id = userId,
					Feedback = "", // or appropriate value
					UserAccount = await _eventService.GetUserAccountAsync(userId),  
					Event = await _eventService.GetEventByIdAsync(eventId)
				};
				await _eventService.AddEventAttendanceAsync(eventAttendance.EventId, eventAttendance.Id);
				return Ok(new
				{
					message = $"Attending event {updatedEvent.Title}",
					button = "<button onclick=\"location.href='/dashboard'\">Back to Dashboard</button>"
				});
			}
			catch (UnauthorizedAccessException ex)
			{
